...
url!=""?url:getUrl(jiek+src.split('"')[0]);\
}else if(src.indexOf("135-cdn")!=-1){\
refreshX5WebView(src);"toast://请等待加载选集！";\
...
eval(mat);\
main.indexOf("http")!=-1?main:link+main;\
}else{\
var main=fc.match(/url:.*?[\'\"](.*?)[\'\"]/)[1];\
main.indexOf("http")!=-1?main:link+main};\
}else if(src.indexOf("meiju11")!=-1){\
fetch(input,{headers:{"Referer":"https://www.meiju11.com"}}).match(/url:.*?[\'\"](.*?)[\'\"]/)[1];\
}else if(src.indexOf("leduotv")!=-1){\
var pla=fetch(src,{}).split("var url=\'")[1].split("\'")[0];\
if(pla.indexOf("m3u8")!=-1){pla.split("=")[1]};\
}else if(src.indexOf("aHR0c")!=-1){decodeURIComponent(base64Decode(src.split("&")[0]));\
...
if(ul.statusCode=="302"){ul.headers.location[0];}else{src};\
...
fetch(sm,{headers:{"Referer":"https://www.shenma4480.com"}}).match(/url:.*?[\'\"](.*?)[\'\"]/)[1].replace(/[+]/g,"%20");\
...
eval(ddjs);\
tools.DdyunPlayer.toUrl(src).replace("ddyunp.com","90mm.me")};\
...
var js=fetch(fileUrl,{});\
eval(js);\
var url=yqjx.toUrl(jiek+pli);url!=""?url:getUrl(jiek+pli);\
